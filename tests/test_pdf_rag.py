...
def test_pdf_parsing():
    ...
    parsed = parse_pdf_bytes(pdf_bytes, filename="sample_test.pdf")

    assert parsed.filename == "sample_test.pdf"
    assert parsed.num_pages >= 1
    assert len(parsed.image_chunks) >= 1
    assert parsed.image_chunks[0].format.upper() in ["PNG", "JPEG", "JPG"]


def test_rag_service_indexing_and_search():
    pdf_bytes = generate_sample_pdf_bytes()
    parsed = parse_pdf_bytes(pdf_bytes, filename="sample_test.pdf")
    ...
    assert index_res["filename"] == "sample_test.pdf"
    assert index_res["images_indexed"] >= 1

    # Test semantic search
    search_res = rag_service.semantic_search("test query", top_k=2)
...
def test_pdf_api_endpoints():
    ...
    assert json_data["filename"] == "sample_test.pdf"
    assert "text_vector_ids" in json_data
    assert "image_vector_ids" in json_data

    # 2. Test POST /api/v1/pdf/query
    query_res = client.post(
        "/api/v1/pdf/query",
        json={"query": "Explain the contents of the uploaded PDF", "top_k": 3},
    )
    assert query_res.status_code == 200
    query_json = query_res.json()
    assert "answer" in query_json
    assert query_json["query"] == "Explain the contents of the uploaded PDF"

    # 3. Test GET /api/v1/pdf/stats
    stats_res = client.get("/api/v1/pdf/stats")
